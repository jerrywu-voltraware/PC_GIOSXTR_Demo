...
def test_app_metadata_uses_requested_version_and_icon():
    ...
    assert APP_VERSION == "V2.0.2"
    assert APP_WINDOW_TITLE == "PC GIOSXTR Demo V2.0.2"
    assert APP_EXECUTABLE_NAME == "PC_GIOSXTR_Demo_V2.0.2"
...
def test_pyinstaller_spec_uses_versioned_exe_icon_and_version_info():
    ...
    assert 'name="PC_GIOSXTR_Demo_V2.0.2"' in spec
    assert 'icon="app_icon.ico"' in spec
    assert 'version="version_info.txt"' in spec
    assert Path("app_icon.ico").exists()
    assert "filevers=(2, 0, 2, 0)" in version_info
    assert "prodvers=(2, 0, 2, 0)" in version_info
    assert "StringStruct('FileVersion', 'V2.0.2')" in version_info
    assert "StringStruct('ProductVersion', 'V2.0.2')" in version_info
    assert "StringStruct('OriginalFilename', 'PC_GIOSXTR_Demo_V2.0.2.exe')" in version_info
