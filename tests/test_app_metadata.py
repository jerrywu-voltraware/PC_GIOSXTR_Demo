...
def test_app_metadata_uses_requested_version_and_icon():
    ...
    assert APP_VERSION == "V1.0.27"
    assert APP_WINDOW_TITLE == "PC GIOSXTR Demo V1.0.27"
...
def test_pyinstaller_spec_uses_versioned_exe_icon_and_version_info():
    ...
    assert 'name="PC_GIOSXTR_Demo_V1.0.27"' in spec
    assert 'icon="app_icon.ico"' in spec
    assert 'version="version_info.txt"' in spec
    assert Path("app_icon.ico").exists()
    assert "filevers=(1, 0, 27, 0)" in version_info
    assert "prodvers=(1, 0, 27, 0)" in version_info
    assert "StringStruct('FileVersion', 'V1.0.27')" in version_info
    assert "StringStruct('ProductVersion', 'V1.0.27')" in version_info
